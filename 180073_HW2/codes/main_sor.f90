...
subroutine sor(mesh, phi, n, tol, lambda)

        implicit none
        real*8 :: mesh(4,n*n), delx, dely, edge1, edge2, total_length, old_residual
        real*8 :: phi(n,n), phi_old(n,n), lambda
        integer :: n, niter, i, j
        ...
        do while(abs(residual(n,phi,phi_old)).gt.tol)
            ...
            do j = 2,n-1
              do i = 2,n-1
                phi(i,j) = (lambda*(g(mesh(3,((j-1)*n)+i),mesh(4,((j-1)*n)+i)) &
                            - (phi(i,j-1)/(dely**2)) &
                            - (phi(i-1,j)/(delx**2)) & 
                            - (phi(i+1,j)/(delx**2)) & 
                            - (phi(i,j+1)/(dely**2))))/(-2.0*((1.0/(delx**2)) + (1.0/(dely**2)))) &
                            + (1.0-lambda)*phi_old(i,j)
              end do
            end do
            ! SOR Loop ends
...
program main
      ...
      ! SOR      
      call sor(mesh, phi, n, tol, lambda)
      error = phi - phi_a
