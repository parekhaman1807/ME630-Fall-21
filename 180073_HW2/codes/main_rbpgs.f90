...
subroutine rbpgs(mesh, phi, n, tol)
        ...
        do while(abs(residual(n,phi,phi_old)).gt.tol)
            ...
            ! Loop to calculate red points using Jacobi starts
            do j = 2,n-1
              m = 2 + mod(j,2)
              do i = m,n-1,2
            ...
            ! Loop to calculate black points using siedel starts
            do j = 2,n-1
              m = 3 - mod(j,2)
              do i = m,n-1,2
                phi(i,j) = (g(mesh(3,((j-1)*n)+i),mesh(4,((j-1)*n)+i)) &
                            ...
                            - (phi(i-1,j)/(delx**2)) & 
                            - (phi(i+1,j)/(delx**2)) & 
                            - (phi(i,j+1)/(dely**2)))/(-2.0*((1.0/(delx**2)) + (1.0/(dely**2))))
              end do
            end do
            ! Loop ends
